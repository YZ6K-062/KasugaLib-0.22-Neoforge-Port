...
public class Saved<T extends SavedData> {
    ...
    T data;
    /**
     * 1.21 requires a {@link HolderLookup.Provider} to (de)serialise SavedData. It is captured from
     * the level when the data is loaded / saved so the public API of this class stays unchanged.
     */
    @Nullable
     ...
     * {@link net.neoforged.neoforge.event.level.LevelEvent.Load} and
     * {@link net.neoforged.neoforge.event.level.LevelEvent.Save}
    ...
    public T loadFromDisk(ServerLevel level) {
        ...
        // 1.21: DimensionDataStorage#computeIfAbsent takes a SavedData.Factory instead of a
        // triple of (loader, supplier, name); the name is passed separately.
        data = level.getDataStorage().computeIfAbsent(
                new SavedData.Factory<>(dataSupplier, (tag, provider) -> {
                    this.registries = provider;
                    return loadFunction.load(tag);
                }),
                resourceKey);
        return data;
    }
     ...
     * {@link net.neoforged.neoforge.event.level.LevelEvent.Save} and
     * {@link net.neoforged.neoforge.event.level.LevelEvent.Unload}
    ...
    public void save(CompoundTag tag) {
        // 1.21: SavedData#save(CompoundTag, HolderLookup.Provider)
        if (data == null || registries == null) return;
    ...
    public void saveToFile(File file) {
        // 1.21: SavedData#save(File, HolderLookup.Provider)
        if (data == null || registries == null) return;
        data.save(file, registries);
    }
