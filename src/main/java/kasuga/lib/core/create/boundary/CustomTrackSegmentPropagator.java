...
import kasuga.lib.core.create.CreateRailway;
...
public class CustomTrackSegmentPropagator{
    public static void propagate(TrackGraph graph, CustomBoundary boundary, boolean direction){
        ...
        GraphExtraData extraData = CreateRailway.server().get().withGraph(graph);
    ...
    public static void walk(
        ...
        if (!forCollection) {
            CreateRailway.server().sync.edgeExtraDataChanged(graph, node1, node2, startEdge, oppositeEdge);
        }
    ...
    private static void walk(
        ...
        GraphExtraData graphExtraData = CreateRailway.server().get().withGraph(graph);
                ...
                for (boolean flip : Iterate.falseAndTrue) {
                    ...
                    if (!extraData.hasCustomBoundaryInThisEdge(featureName)) {
                        if (nonBoundaryCallback.test(extraData)) {
                            CreateRailway.server().sync.edgeExtraDataChanged(graph, currentNode, nextNode, edge, oppositeEdge);
                        }
                        continue;
                    }
