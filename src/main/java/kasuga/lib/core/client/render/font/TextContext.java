...
public class TextContext {
    ...
    public void renderToGui(PoseStack pose) {
        ...
        pose.mulPose(new Quaternionf().rotationXYZ(this.rotation.x(), this.rotation.y(), this.rotation.z()));
        pose.translate(- pivot.x() * this.getWidth(), - pivot.y() * this.getHeight(), 0);
        pose.scale(this.scale.x(), this.scale.y(), 1f);

        Minecraft.getInstance().font.drawInBatch(text, 0, 0, color.getRGB(), false, pose.last().pose(), Minecraft.getInstance().renderBuffers().bufferSource(), net.minecraft.client.gui.Font.DisplayMode.NORMAL, 0, 15728880);

        pose.scale(1 / this.scale.x(), 1 / this.scale.y(), 1f);
        pose.translate(pivot.x() * this.getWidth(), pivot.y() * this.getHeight(), 0);
        Vector3f negRot = new Vector3f(rotation);
        negRot.mul(-1f);
        pose.mulPose(new Quaternionf().rotationXYZ(negRot.x(), negRot.y(), negRot.z()));
        pose.translate(- position.x(), - position.y(), - position.z());
    }
    ...
    public void renderToWorld(PoseStack pose, MultiBufferSource source, boolean dropShadow, boolean transparent, SimpleColor bgColor, int light) {
        ...
        Minecraft.getInstance().font.drawInBatch(text, 0 ,0, color.getRGB(), dropShadow, pose.last().pose(),
                source, transparent ? net.minecraft.client.gui.Font.DisplayMode.SEE_THROUGH : net.minecraft.client.gui.Font.DisplayMode.NORMAL, bgColor.getRGB(), light);
