...
public class SimpleTexture {
    ...
    public SimpleTexture(@NotNull ResourceLocation location, int uOffset, int vOffset, int uWidth, int vHeight, SimpleColor color, BufferedImage image, byte[] bytesOfImage){
    ...
    public SimpleTexture(@NotNull ResourceLocation location, int uOffset, int vOffset, int uWidth, int vHeight, int color, float alpha) {
        this.location = location;
        this.uOffset = uOffset;
        this.vOffset = vOffset;
        this.uWidth = uWidth;
    ...
    public SimpleTexture(@NotNull ResourceLocation location, InputStream stream, int uOffset, int vOffset, int uWidth, int vHeight, int color, float alpha) {
        this.location = location;
        this.uOffset = uOffset;
        this.vOffset = vOffset;
        this.uWidth = uWidth;
        ...
        try {
            ...
            Minecraft.getInstance().getTextureManager().register(location, new DynamicTexture(NativeImage.read(readStream)));
    ...
    public void uploadPicture(@NotNull ResourceLocation location) {
        try {
    ...
    public SimpleTexture(@NotNull ResourceLocation location, int uOffset, int vOffset, int uWidth, int vHeight, int color) {
        this(location, uOffset, vOffset, uWidth, vHeight, color, 1.0f);
    }

    public SimpleTexture(@NotNull ResourceLocation location, int uOffset, int vOffset, int uWidth, int vHeight) {
        this(location, uOffset, vOffset, uWidth, vHeight, COLOR_DEFAULT, 1.0f);
    }

    public SimpleTexture(@NotNull ResourceLocation location, int uWidth, int vHeight, int color, float alpha) {
        this(location, 0, 0, uWidth, vHeight, color, alpha);
    }

    public SimpleTexture(@NotNull ResourceLocation location, int uWidth, int vHeight, int color) {
        this(location, 0, 0, uWidth, vHeight, color, 1.0f);
    }

    public SimpleTexture(@NotNull ResourceLocation location, int uWidth, int vHeight) {
        this(location, 0, 0, uWidth, vHeight, COLOR_DEFAULT, 1.0f);
    }

    public SimpleTexture(@NotNull ResourceLocation location, int color, float alpha) {
    ...
    public SimpleTexture(@NotNull ResourceLocation location, int color) {
        this(location, color, 1.0f);
    }

    public SimpleTexture(@NotNull ResourceLocation location, float alpha) {
        this(location, COLOR_DEFAULT, alpha);
    }

    public SimpleTexture(@NotNull ResourceLocation location) {
    ...
    public static SimpleTexture fromByteBuf(FriendlyByteBuf buf) {
        ...
        CompoundTag colorTag = buf.readNbt();
    ...
    public void render(int x, int y, int width, int height) {
        ...
        buffer.addVertex(x, y, 0f).setUv(fuOffset, fvOffset);
        buffer.addVertex(x, y + height, 0f).setUv(fuOffset, fvOffset + fvHeight);
        buffer.addVertex(x + width, y + height, 0f).setUv(fuOffset + fuWidth, fvOffset + fvHeight);
        buffer.addVertex(x + width, y, 0f).setUv(fuOffset + fuWidth, fvOffset);
    ...
    public void renderUV(int x, int y, int width, int height, float baseU, float baseV, float additionU, float additionV){
        ...
        RenderSystem.enableBlend();
        BufferBuilder buffer = Tesselator.getInstance().begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX);
        buffer.addVertex(x, y, 0f).setUv(baseU, baseV);
        buffer.addVertex(x, y + height, 0f).setUv(baseU, baseV + additionV);
        buffer.addVertex(x + width, y + height, 0f).setUv(baseU + additionU, baseV + additionV);
        buffer.addVertex(x + width, y, 0f).setUv(baseU + additionU, baseV);
        BufferUploader.drawWithShader(buffer.buildOrThrow());
        RenderSystem.disableBlend();
    }
