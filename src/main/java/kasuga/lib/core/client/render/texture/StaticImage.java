...
public class StaticImage {
    ...
    public static Supplier<StaticImage> createImage(CompoundTag nbt) throws IOException {
        return createImage(
                ResourceLocation.fromNamespaceAndPath(nbt.getString("id_namespace"), nbt.getString("id_path")),
    ...
    public static Supplier<StaticImage> createImage(ResourceLocation id, InputStream inputStream) throws IOException {
        ...
        StaticImage image = new StaticImage(cheat.location(), inputStream);
    ...
    private static DynamicTexture registerImageToMc(Resources.CheatResourceLocation id, InputStream stream) throws IOException {
        ...
        NativeImage image = NativeImage.read(stream);
        DynamicTexture texture = new DynamicTexture(image);
        Minecraft.getInstance().getTextureManager().register(id.location(), texture);
        Resources.CHEAT_TEXTURES.add(id.location());
        return texture;
    }
    ...
    public void renderToGui(Vector3f leftTop, Vector3f rightTop, Vector3f leftDown, Vector3f rightDown,
        ...
        buffer.addVertex(leftTop.x(), leftTop.y(), leftDown.z()).setUv(uvLeftTop.x(), uvLeftTop.y());
        buffer.addVertex(leftDown.x(), leftDown.y(), leftDown.z()).setUv(uvLeftDown.x(), uvLeftDown.y());
        buffer.addVertex(rightDown.x(), rightDown.y(), rightDown.z()).setUv(uvRightDown.x(),uvRightDown.y());
        buffer.addVertex(rightTop.x(), rightTop.y(), rightTop.z()).setUv(uvRightTop.x(), uvRightTop.y());
    ...
    public void renderToGui(PoseStack.Pose pose, Vector3f leftTop, Vector3f rightTop, Vector3f leftDown, Vector3f rightDown,
        ...
        buffer.addVertex(poseMatrix, leftTop.x(), leftTop.y(), leftDown.z()).setUv(uvLeftTop.x(), uvLeftTop.y());
        buffer.addVertex(poseMatrix, leftDown.x(), leftDown.y(), leftDown.z()).setUv(uvLeftDown.x(), uvLeftDown.y());
        buffer.addVertex(poseMatrix, rightDown.x(), rightDown.y(), rightDown.z()).setUv(uvRightDown.x(),uvRightDown.y());
        buffer.addVertex(poseMatrix, rightTop.x(), rightTop.y(), rightTop.z()).setUv(uvRightTop.x(), uvRightTop.y());
        BufferUploader.drawWithShader(buffer.buildOrThrow());
        RenderSystem.disableBlend();
    }
    ...
    private void buildVertex(VertexConsumer consumer, Matrix4f matrix,
        ...
        consumer.addVertex(matrix, pos.x(), pos.y(), pos.z())
                .setColor(color.getfR(), color.getfG(), color.getfB(), color.getA())
                .setUv(uv.x(), uv.y())
                .setLight(light);
