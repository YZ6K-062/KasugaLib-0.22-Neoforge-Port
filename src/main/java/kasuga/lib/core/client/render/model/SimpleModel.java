...
public class SimpleModel {
    ...
    public SimpleModel(@NotNull String key, BakedModel model) {
        this.key = key;
        this.model = model;
    ...
    public SimpleModel(@NotNull ModelReg reg) {
    ...
    public void render(PoseStack pose, MultiBufferSource source, float x, float y, float z, int light, int overlay) {
        ...
        pose.pushPose();
        if(useParentPose && lastMatrix != null) pose.last().pose().mul(lastMatrix);
        context.apply(pose);
    ...
    void renderModel(PoseStack pose, VertexConsumer consumer, List<BakedQuad> quads, SimpleColor color, int light, int overlay) {
        for(BakedQuad bakedquad : quads) {
            consumer.putBulkData(pose.last(), bakedquad, color.getfR(), color.getfG(), color.getfB(), color.getA(), light, overlay);
        }
    }
