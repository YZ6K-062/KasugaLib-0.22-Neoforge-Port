...
public class RenderableItem {
    ...
    public void render(@Nullable PoseStack pose, ItemRenderer renderer, int x, int y, int width, int height){
        ...
        if(renderCount && stack.isStackable() && stack.getCount() > 1 && pose != null) {
            ...
            font.drawInBatch(count, x + width - w, y + height - font.lineHeight, 0xffffff, false, pose.last().pose(), Minecraft.getInstance().renderBuffers().bufferSource(), Font.DisplayMode.NORMAL, 0, 15728880);
        }
    }
    ...
    protected void renderGuiItem(
        ...
        // 1.21: the model-view stack is a JOML Matrix4fStack (pushMatrix/popMatrix).
        Matrix4fStack posestack = RenderSystem.getModelViewStack();
        posestack.pushMatrix();
        posestack.translate(x, y, 0);
        posestack.translate(((float) width) / 2, ((float) height) / 2, 0.0F);
