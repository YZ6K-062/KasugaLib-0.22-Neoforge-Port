...
public class ImageMask {
    ...
    public ImageMask(String name, CompoundTag nbt) {
        ...
        image = StaticImage.STACK.get(ResourceLocation.parse(tag.getString("id"))).get();
    ...
    public ImageMask rotateByPivot(Vector3f pivot, Vector3f rotationRad) {
        Quaternionf quaternion = new Quaternionf().rotationXYZ(rotationRad.x(), rotationRad.y(), rotationRad.z());
    ...
    public ImageMask flipByZero(Axis axis) {
        return flip(new Vector3f(0, 0, 0), axis);
    }
    ...
    public ImageMask scaleByZero(float factor) {
        return scale(new Vector3f(0, 0, 0), factor);
    }
