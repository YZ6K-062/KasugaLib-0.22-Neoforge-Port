...
public class ModelMappings {
    ...
    public void map() throws IOException {
        Map<String, Resource> resources = Resources.getFullPathResources(ResourceLocation.fromNamespaceAndPath(namespace, IDENTIFIER), false);
        for(String name : resources.keySet()) {
            ...
            map.put(ResourceLocation.fromNamespaceAndPath(namespace, plainName), ResourceLocation.fromNamespaceAndPath(namespace, name));
        }
        map_finished = true;
    }

    public void addMapping(ResourceLocation location0, ResourceLocation location1) {
        ResourceLocation location = ResourceLocation.fromNamespaceAndPath(location1.getNamespace(), location1.getPath());
