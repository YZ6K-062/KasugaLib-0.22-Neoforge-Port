...
public class BoneMovement extends AnimationElement implements AnimAssignable, IAnchor {
    ...
    public void setCondition(@NotNull String logical) {
        this.condition.fromString(Objects.requireNonNull(logical));
    }

    public void setDisplayCondition(@NotNull String condition) {
    ...
    public PoseContext.Action xRot(Formula fx) {
        return  (pose) -> {pose.mulPose(new Quaternionf().rotateX((float) Math.toRadians(fx.getResult())));};
    }
    public PoseContext.Action yRot(Formula fy) {
        return  (pose) -> {pose.mulPose(new Quaternionf().rotateY((float) Math.toRadians(fy.getResult())));};
    }
    public PoseContext.Action zRot(Formula fz) {
        return  (pose) -> {pose.mulPose(new Quaternionf().rotateZ((float) Math.toRadians(fz.getResult())));};
    }

    public PoseContext.Action xRotRad(Formula fx) {
        return  (pose) -> {pose.mulPose(new Quaternionf().rotateX(fx.getResult()));};
    }

    public PoseContext.Action yRotRad(Formula fy) {
        return  (pose) -> {pose.mulPose(new Quaternionf().rotateY(fy.getResult()));};
    }

    public PoseContext.Action zRotRad(Formula fz) {
        return  (pose) -> {pose.mulPose(new Quaternionf().rotateZ(fz.getResult()));};
    }
    ...
    public PoseContext.Action pointTo(IAnchor anchor) {
        return (pose) -> {
            ...
            pose.mulPose(new Quaternionf().rotateX((float) Math.toRadians(context2.x_rot() - context1.x_rot())));
            pose.mulPose(new Quaternionf().rotateY((float) Math.toRadians(context2.y_rot() - context1.y_rot())));
            pose.mulPose(new Quaternionf().rotateZ((float) Math.toRadians(context2.z_rot() - context1.z_rot())));
        };
    }

    public PoseContext.Action pointTo(Formula px, Formula py, Formula pz) {
        return (pose) -> {
            pose.mulPose(new Quaternionf().rotateX((float) Math.toRadians(px.getResult())));
            pose.mulPose(new Quaternionf().rotateY((float) Math.toRadians(py.getResult())));
            pose.mulPose(new Quaternionf().rotateZ((float) Math.toRadians(pz.getResult())));
        };
    }
