...
public class Animation {
    public static final KeyFrameHolder<Translation> TRANSLATIONS = new KeyFrameHolder<Translation>(
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "linear_translation"), new KeyFrame<LinearTranslation>(
                    ((current, next) -> new LinearTranslation(next.getData().subtract(current.getData()), current.getTime(), next.getTime()))
            )),
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "bezier_translation"), new KeyFrame<BezierTranslation>(
                    ((current, next) -> new BezierTranslation(next.getData().subtract(current.getData()), current.getTime(), next.getTime()))
            )),
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "custom_translation"), new KeyFrame<CustomTranslation>(
    ...
    public static final KeyFrameHolder<Rotation> ROTATIONS = new KeyFrameHolder<Rotation>(
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "linear_rotation"), new KeyFrame<LinearRotation>(
                    (current, next) -> new LinearRotation(next.getData().subtract(current.getData()), current.getTime(), next.getTime(), false)
            )),
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "bezier_rotation"), new KeyFrame<BezierRotation>(
                    (current, next) -> new BezierRotation(next.getData().subtract(current.getData()), current.getTime(), next.getTime(), false)
            )),
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "custom_rotation"), new KeyFrame<CustomRotation>(
    ...
    public static final KeyFrameHolder<Scaling> SCALINGS = new KeyFrameHolder<Scaling>(
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "linear_scaling"), new KeyFrame<LinearScaling>(
                    (current, next) -> new LinearScaling(next.getData().subtract(current.getData()), current.getTime(), next.getTime())
            )),
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "bezier_scaling"), new KeyFrame<BezierScaling>(
                    (current, next) -> new BezierScaling(next.getData().subtract(current.getData()), current.getTime(), next.getTime())
            )),
            Pair.of(ResourceLocation.fromNamespaceAndPath(KasugaLib.MOD_ID, "custom_scaling"), new KeyFrame<CustomScaling>(
