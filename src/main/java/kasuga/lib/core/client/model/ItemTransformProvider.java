...
public interface ItemTransformProvider {
    @Nullable
    default HashMap<ItemDisplayContext, ItemTransform> generate(JsonObject jsonObject, Type type, JsonDeserializationContext context) {
        ResourceLocation ml = ResourceLocation.parse(jsonObject.get("model").getAsString());
        JsonArray geoJson;
        try {
            Resource resource = Resources.getResource(ResourceLocation.fromNamespaceAndPath(ml.getNamespace(), "models/" + ml.getPath() + ".geo.json"));
