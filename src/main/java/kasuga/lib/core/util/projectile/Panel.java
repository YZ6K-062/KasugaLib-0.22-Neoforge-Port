...
public class Panel {
    ...
    public Panel rotate(Quaternionf quaternion, Vec3 referencePoint) {
        ...
        Vector3f neoNormal = new Vector3f((float) normal.x(), (float) normal.y(), (float) normal.z());
        neoNormal.rotate(quaternion);
        return new Panel(referencePoint, new Vec3(neoNormal));
    }

    public Panel rotateDeg(Vector3f rotationDeg, Vec3 referencePoint) {
        return rotate(new Quaternionf().rotationXYZ((float) java.lang.Math.toRadians(rotationDeg.x()), (float) java.lang.Math.toRadians(rotationDeg.y()), (float) java.lang.Math.toRadians(rotationDeg.z())), referencePoint);
    }

    public Panel rotate(Vector3f rotation, Vec3 referencePoint) {
        return rotate(new Quaternionf().rotationXYZ(rotation.x(), rotation.y(), rotation.z()), referencePoint);
    }
    ...
    public Vector3f getHitPoint(Vector3f pos, Vector3f forward) {
        return new Vector3f((float) getHitPoint(new Vec3(pos), new Vec3(forward)).x(), (float) getHitPoint(new Vec3(pos), new Vec3(forward)).y(), (float) getHitPoint(new Vec3(pos), new Vec3(forward)).z());
    }
    ...
    public Vector3f map(Vec2f vec) {
        ...
        Quaternionf quaternion = new Quaternionf().rotationXYZ(0, pitchAndYaw.y(), pitchAndYaw.x());
    ...
    public static Vec2f getPitchAndYaw(Vec3 vec) {
        ...
        float pitch = vecLen == 0 ? 0 : (float) java.lang.Math.asin(horizontal.length() / vecLen);
    ...
    public static Quaternionf getQuaternion(Vec3 vec) {
        ...
        Quaternionf yaw = new Quaternionf().rotationXYZ(0, pitchAndYaw.y(), 0);
        Quaternionf pitch = new Quaternionf().rotationXYZ(0, 0, pitchAndYaw.x());
        result.mul(yaw);
        result.mul(pitch);
        return result;
    }
    ...
    public Vector3f getRotationVector() {
        return getQuaternion().getEulerAnglesXYZ(new Vector3f());
    }

    public Vector3f getRotationDegVector() {
        return getQuaternion().getEulerAnglesXYZ(new Vector3f()).mul((float) (180.0 / java.lang.Math.PI));
    }
