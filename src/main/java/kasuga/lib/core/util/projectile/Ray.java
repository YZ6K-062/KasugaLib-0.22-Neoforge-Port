...
public class Ray {
    ...
    public Ray(Vec3 source, Vec3 forward) {
        this(new Vector3f((float) source.x(), (float) source.y(), (float) source.z()), new Vector3f((float) forward.x(), (float) forward.y(), (float) forward.z()));
    }

    public void changeDirection(Quaternionf quaternion) {
        forward.rotate(quaternion);
    }
    ...
    public Vector3f get(float distance) {
        Vector3f result = new Vector3f((float) source.x(), (float) source.y(), (float) source.z());
        Vector3f f = new Vector3f((float) forward.x(), (float) forward.y(), (float) forward.z());
        f.mul(distance);
        result.add(f);
        return result;
    }
    ...
    public Ray reflect(final Vector3f normal, Vector3f neoSource) {
        ...
        return new Ray(neoSource, new Vector3f((float) mir.x(), (float) mir.y(), (float) mir.z()));
    }

    public Ray reflect(Panel panel) {
        if (panel.parallel(this.forward)) return this;
        if (panel.isPointOnPanel(this.source))
            return reflect(new Vector3f((float) panel.normal.x(), (float) panel.normal.y(), (float) panel.normal.z()), this.source);
        Vector3f hitPoint = panel.getHitPoint(this);
        Vector3f test = new Vector3f(hitPoint);
        test.sub(source);
        if (test.dot(forward) < 0) return this;
        return reflect(new Vector3f((float) panel.normal.x(), (float) panel.normal.y(), (float) panel.normal.z()), hitPoint);
    }
    ...
    public boolean pointOnRay(Vector3f vector) {
        ...
        test.cross(forward);
        if (!test.equals(new Vector3f(0, 0, 0))) return false;
