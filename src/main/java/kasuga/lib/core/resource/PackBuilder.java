...
public class PackBuilder {
    ...
    public Pack create() {
        PackLocationInfo info = new PackLocationInfo(this.id, title, source, java.util.Optional.empty());
        Pack.Metadata metadata = new Pack.Metadata(description, compatibility,
                net.minecraft.world.flag.FeatureFlagSet.of(), List.of());
        PackSelectionConfig selection = new PackSelectionConfig(required, defaultPosition, fixedPos);
        Pack.ResourcesSupplier supplier = new Pack.ResourcesSupplier() {
            ...
            @Override
            public PackResources openFull(PackLocationInfo location, Pack.Metadata meta) {
                return packSupplier.get();
            }
        };
        return new Pack(info, supplier, metadata, selection);
    }
}
