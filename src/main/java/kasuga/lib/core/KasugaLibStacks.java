...
public class KasugaLibStacks {
    ...
    public KasugaLibStacks(IEventBus bus) {
        ...
        ARGUMENT_TYPES = DeferredRegister.create(Registries.COMMAND_ARGUMENT_TYPE, MOD_ID);
        ...
        NeoForge.EVENT_BUS.addListener(ServerStartingEvents::serverStarting);
        NeoForge.EVENT_BUS.addListener(ServerStartingEvents::serverAboutToStart);
        NeoForge.EVENT_BUS.addListener(ServerStartingEvents::serverStopped);
        NeoForge.EVENT_BUS.addListener(ResourcePackEvent::onResourcePackReload);
        // RegisterClientReloadListenersEvent is a client-side MOD bus event in 1.21.x;
        // attaching it to NeoForge.EVENT_BUS (the common game bus) throws
        //   IModBusEvent events are not allowed on the common NeoForge bus
        // at mod-construction time, so it has to go on the per-mod bus.
        bus.addListener(ResourcePackEvent::onClientResourcePackReload);

        NeoForge.EVENT_BUS.addListener(ServerChunkMenuLocatorManager::onWatch);
        NeoForge.EVENT_BUS.addListener(ServerChunkMenuLocatorManager::onUnWatch);
        bus.addListener(BothSetupEvent::onFMLCommonSetup);
        bus.addListener(EntityAttributeEvent::entityAttributeCreation);
        NeoForge.EVENT_BUS.addListener(ServerTickEvent::onServerTick);
        ...
        if(Envs.isClient()) {
            ...
            NeoForge.EVENT_BUS.addListener(Constants::onClientTick);
            NeoForge.EVENT_BUS.addListener(Constants::onAnimStart);
            NeoForge.EVENT_BUS.addListener(Constants::onAnimStop);
            NeoForge.EVENT_BUS.addListener(ClientTickEvent::onClientTick);
            NeoForge.EVENT_BUS.addListener(ClientTickEvent::onGuiTick);

            NeoForge.EVENT_BUS.addListener(PlayLogEvent::playerLogout);
            NeoForge.EVENT_BUS.addListener(PlayLogEvent::playerLogin);
            NeoForge.EVENT_BUS.addListener(ClientConnection::onClientConnect);
            NeoForge.EVENT_BUS.addListener(ClientConnection::onClientDisconnect);
            ...
            // 1.21: menu screens are contributed through RegisterMenuScreensEvent, which is fired by
            // MenuScreens.init() during client boot — so the listener has to be attached here.
            bus.addListener(ClientSetupEvent::onRegisterMenuScreens);
            NeoForge.EVENT_BUS.addListener(RenderTickEvent::onRenderTick);
            NeoForge.EVENT_BUS.addListener(InteractionFovEvent::onComputedFov);
        ...
        NeoForge.EVENT_BUS.addListener(ServerResourceListener::onServerStarting);
        NeoForge.EVENT_BUS.addListener(ServerResourceListener::onServerStopping);
        NeoForge.EVENT_BUS.addListener(ServerConnectionListeners::onClientDisconnect);
