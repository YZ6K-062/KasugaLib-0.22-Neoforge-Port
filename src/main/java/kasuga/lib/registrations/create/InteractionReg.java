        ...
        switch (workType) {
            case blocks -> {
                ...
                for (BlockReg block : blocks)
                    // Create 6.0: AllInteractionBehaviours.registerBehaviour -> MovingInteractionBehaviour.REGISTRY.register
            ...
            case state_predicate -> {
                if (statePredicate == null)
                    crashOnNotPresent(InteractionReg.class, getIdentifier(), "you must provide a predictor for registration.");
                // Create 6.0: registerBehaviourProvider -> registerProvider(Provider<Block, V>).
