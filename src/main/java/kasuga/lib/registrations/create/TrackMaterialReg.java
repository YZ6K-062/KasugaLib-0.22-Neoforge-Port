...
public class TrackMaterialReg extends Reg {
    ...
    public TrackMaterialReg submit(SimpleRegistry registry) {
        ...
        if (isCustomRendered)
            // Create 6.0: customModels now takes Supplier<Supplier<PartialModel>>; the outer
            // `() -> tieModel` (tieModel is already a Supplier<PartialModel>) satisfies that.
            factory.customModels(() -> tieModel, () -> leftSegModel, () -> rightSegModel);
        else
            factory.defaultModels();

        material = factory.build();
        CreateRailway.cacheTrackMaterialIn(this);
        return this;
    }
