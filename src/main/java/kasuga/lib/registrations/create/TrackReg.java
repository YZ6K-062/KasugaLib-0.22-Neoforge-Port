...
public class TrackReg<T extends TrackBlock> extends BlockReg<T> {
    ...
    /**
     * Optional. Sets the item model location (passed to {@code ItemModelProvider#generated}).
     * If never called, the track item falls back to the default block-derived item model
     * instead of crashing on a null texture during datagen.
     */
    public TrackReg<T> trackItemModel(ResourceLocation location) {
        this.trackItemModelLocation = location;
        return this;
    }
    ...
    public TrackReg<T> submit(SimpleRegistry registry) {
        ...
        var itemBuilder = builder.onRegister(CreateRegistrate.blockModel(()->TrackModelDeduplicator.simple(TrackModel::new)))
                .item(TrackBlockItem::new);
        if (trackItemModelLocation != null)
            itemBuilder.model((c, p) -> p.generated(c, trackItemModelLocation));
        else
            // No explicit item model set: fall back to a generated item model using the
            // item's own texture (mirrors Registrate's ItemBuilder.defaultModel()).
            // Avoids BlockBuilder's default blockItem() which parents to block/<name>,
            // a model track blockstates do not produce -> "Model at ... does not exist".
            itemBuilder.model((c, p) -> p.generated(c, p.itemTexture(c)));
        itemBuilder.build();
        entry = builder.register();
        // NOTE: do NOT also push the item into the CreativeTabReg here.
        // registrate.setCreativeTab(...) above already registers a
        // BuildCreativeModeTabContentsEvent listener that accepts every item built by this
        // Registrate into that tab. Adding it a second time through CreativeTabReg#item()
        // made NeoForge abort the whole tab build with
        //   IllegalArgumentException: Itemstack 1 <mod>:<track> already exists in the tab's list
        // which crashed world load. (It also NPE'd when no tab was configured.)
        return this;
    }
