        ...
        switch (workType) {
            case blocks -> {
                ...
                for (BlockReg block : blocks)
                    // Create 6.0: AllMovementBehaviours.registerBehaviour -> MovementBehaviour.REGISTRY.register
            ...
            case state_predicate -> {
                if (statePredicate == null)
                    crashOnNotPresent(MovementReg.class, getIdentifier(), "you must provide a predictor for registration.");
                // Create 6.0: registerBehaviourProvider(Predicate<BlockState>) -> registerProvider(Provider<Block, V>).
                // The provider receives a Block (not BlockState); we evaluate against its default state
                // to preserve the original statePredicate semantics as closely as possible.
                MovementBehaviour.REGISTRY.registerProvider(
                        block -> statePredicate.test(block.defaultBlockState()) ? behaviour : null
                );
            }
            case tag -> {
                if (tags == null)
