...
public class ModelReg extends Reg {
    ...
    public void compileFile(SimpleRegistry registry) throws IOException {
        ResourceLocation location = ResourceLocation.fromNamespaceAndPath(registry.namespace, "models/" + this.location.getPath() + ".json");
    ...
    private ResourceLocation getModelLocation(ResourceLocation location) {
        return ResourceLocation.fromNamespaceAndPath(location.getNamespace(), "models/" + location.getPath() + ".json");
    }
    ...
    private static ArrayList<Map.Entry<String, ResourceLocation>> getMapFromJson(JsonObject members, String prefix) {
        ...
        for(String key : m.keySet()) {entries.add(new AbstractMap.SimpleEntry<>(former + key, ResourceLocation.parse(m.get(key))));}
        return entries;
    }
